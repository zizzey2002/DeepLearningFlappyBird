...
# -*- coding: utf-8 -*-
"""
Flappy Bird DQN 2025 现代修复版（整合最终版）

特点：
- 使用 TensorFlow 1.x 风格（tf.compat.v1）
- 改进超参数，提升收敛速度
...
EXPLORE = 300000     # epsilon 从 1.0 衰减到 0.1 所花的步数
INITIAL_EPSILON = 0.5
FINAL_EPSILON = 0.05
...
def trainNetwork(s, readout, h_fc1, sess):
    ...
    train_step = tf.compat.v1.train.AdamOptimizer(LEARNING_RATE).minimize(cost)

    # ----- 2. 构建 target network 参数同步操作 -----
    all_vars = tf.compat.v1.trainable_variables()
    num_vars = len(all_vars)
    # 假设前一半是主网络变量，后一半是目标网络变量
    main_vars = all_vars[:num_vars // 2]
    target_vars = all_vars[num_vars // 2:]
    ...
    print("开始训练！小鸟要起飞了🚀（target network 已初始化）")
    ...
    while True:
        ...
        if t > OBSERVE:
            ...
            # 用 target 网络计算 Q(s', a') 最大值
            readout_j1_batch = sess.run(readout_target, feed_dict={s_target: s_j1_batch})
...
def playGame():
    ...
    s, readout, h_fc1 = createNetwork()   # 主网络
